...
internal class CaseDig
{
    ...
    private static Bom createBom()
    {
        ...
        bD1G2.SceneCondition = scene =>
            scene.Variables.TryGetValue("Count_D1G1", out Variable getVar) && getVar == N1;
        bD1G3.AddSubBom(bD1G2);
        bD1G3.SceneCondition = scene =>
            scene.Variables.TryGetValue("Count_D1G2", out Variable getVar) && getVar == N1;

        var bD2G1 = new Bom("D2G1");
        var bD2G2 = new Bom("D2G2");
        bD2G2.AddSubBom(bD2G1);
        bD2G2.SceneCondition = scene =>
            scene.Variables.TryGetValue("Count_D2G1", out Variable getVar) && getVar == N2;
        ...
        bD3G2.SceneCondition = scene =>
            scene.Variables.TryGetValue("Count_D3G1", out Variable getVar) && getVar == N3;
        bD3G3.AddSubBom(bD3G2);
        bD3G3.SceneCondition = scene =>   //D3G2的数量达到要求，才能进行D3G3
            scene.Variables.TryGetValue("Count_D3G2", out Variable getVar) && getVar == N3;
